...
export async function createACCTemplate(workbook, { month, year, payslips, periodType, EmployerDetails }) {
  const worksheet = workbook.addWorksheet('ACC Contribution', {
  ...
  titleCell.value = 'ACCIDENT COMPENSATION CORP';
  ...
  const startRow = 15;
  organizedPayslips.forEach((payslip, index) => {
    const row = worksheet.addRow([
      payslip.employeeId,
      payslip.employeeName,
      ...generatePeriodValues(payslip, periodType),
      calculateTotalACC(payslip, periodType)
    ]);
...
function organizePayslips(payslips, periodType) {
  ...
  payslips.forEach(payslip => {
    ...
    entry.periods.set(periodNum, (payslip.payrollBreakdown.deductions.acc || 0));
    entry.totalACC += (payslip.payrollBreakdown.deductions.acc || 0);
  });
...
function calculateTotalACC(payslip, periodType) {
  return Array.from(payslip.periods.values()).reduce((sum, value) => sum + value, 0);
}