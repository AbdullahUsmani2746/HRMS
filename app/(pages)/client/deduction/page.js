...
const Deduction = () => {
  ...
  const handleEdit = async (updatedData) => {
    try {
      const response = await axios.put(`/api/employee/deduction/${updatedData._id}`, updatedData);
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Deduction</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        ...
        <div className="transition-width duration-300 flex-1 p-6">
          <div className="p-4">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-2xl">Deduction</h1>
              <Button onClick={() => openModal()}>Add Deduction</Button>
            </div>
            <Table className="shadow-md rounded-lg border-separate">
              <TableHeader>
                <TableRow className="bg-foreground text-left">
                  ...
                  <TableHead className="px-4 py-2 font-semibold text-white">Deduction Description</TableHead>
                  <TableHead className="px-4 py-2 font-semibold text-white">Deduction Rate</TableHead>
              ...
              <TableBody>
                {data.map(single => (
                  <TableRow key={single._id} className="bg-background shadow-lg rounded-lg border-separate" > 
                    <TableCell className="px-4">{single.deduction}</TableCell>
                    <TableCell className="px-4">{single.deduction_description}</TableCell>
                    <TableCell className="px-4">{single.deduction_rate}</TableCell>
                    <TableCell>
