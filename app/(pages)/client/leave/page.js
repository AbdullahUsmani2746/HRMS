...
const Leave = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/employees/leave/${id}`);
  ...
  const handleEdit = async (updatedData) => {
    try {
      const response = await axios.put(`/api/employee/leave/${updatedData._id}`, updatedData);
        ...
        <div className="transition-width duration-300 flex-1 p-6">
          <div className="p-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <Button onClick={() => openModal()}>AddLeave</Button>
            </div>
            <Table className="shadow-md rounded-lg border-separate">
              <TableHeader>
                <TableRow className="bg-foreground text-left">
                  ...
                  <TableHead className="px-4 py-2 font-semibold text-white">Leave Description</TableHead>
              ...
              <TableBody>
                {data.map(single => (
                  <TableRow key={single._id} className="bg-background shadow-lg rounded-lg border-separate" > 
                    <TableCell className="px-4">{single.leave}</TableCell>
                    <TableCell className="px-4">{single.leave_description}</TableCell>
                    <TableCell>
