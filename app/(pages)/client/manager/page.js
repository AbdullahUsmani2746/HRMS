...
const DynamicComponent = ({ existingData, onClose }) => {
  ...
  useEffect(() => {
    if (selectedDepartment) {
      const fetchEmployeeData = async () => {
        try {
          const response = await axios.get(`/api/employees?departmentId=${selectedDepartment}`);
  ...
  const handleFieldChange = (field, value, formData, setFormData) => {
    if (field === 'departmentId') {
      ...
      const selectedEmployee = employeeData.find(emp => emp.employeeId === value._id);
      if (selectedEmployee) {
  ...
  const handleSubmit = async (data, isEditing, editIndex) => {
    try {
      ...
      if (isEditing) {
        await axios.put(`/api/employees/manager/${data[editIndex]._id}`, data[editIndex]);
      } else {
